...
namespace DevCruise.Blazor.Client.Shared
{
    ...
    public static class Reducers<TM>
    where TM:MyStateBase, new()
    {
        public static TM RootReducer(TM state, IAction action)
        {
            if (state == null)
            ...
            return new TM
            {
                ...
                ApiRoot = ApiRootReducer(state.ApiRoot, action)
            };
        }
        ...
        private static string ApiRootReducer(string apiRoot, IAction action)
        {
            switch (action)
            {
                default:
